...
function tsdb_run_test {
    ...
    import_file_path="/home/mildronize/gits/csv-to-opentsdb/generated-dps/dps-$id"
    ...
    tsdb_import $id dps-$id && \
        ...
        total_dps_error=$(tsdb_count_errors)
    import_file_size=$(tsdb_get_file_size $import_file_path)
    hbase_storage_size=$(tsdb_get_file_size $hbase_storage_path)
    # write to csv
    echo $id,$total_dps_fsck,$total_dps_scan,$total_dps_error,$hbase_storage_size >> $IMPORT_LOG_PATH
    # total_dps_fsck=$(tsdb_count_dps_fsck $id $nthread)
